...
import { safeBase32Encode, safeBase32Decode } from '../../utils/converters';

type Mode = 'encode' | 'decode';

export default function TextBase32() {
  ...
  const processConversion = (val: string, currentMode: Mode) => {
    ...
    try {
      if (currentMode === 'encode') {
        setOutput(safeBase32Encode(val));
        setError('');
      } else {
        setOutput(safeBase32Decode(val));
        setError('');
      }
    } catch (e: any) {
      setError(currentMode === 'encode' ? 'Failed to encode text' : 'รูปแบบ Base32 ไม่ถูกต้อง (RFC 4648)');
      setOutput('');
    }
  };
  ...
  return (
    <div className="relative flex flex-col items-center w-full min-h-full py-8 md:py-12 z-10">
      ...
      <div className="w-full max-w-4xl flex flex-col md:flex-row md:justify-between md:items-end mb-8 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-text mb-2 tracking-tight">Text Base32</h1>
          <p className="text-muted font-medium">Encode text to Base32 format (RFC 4648) or decode it back.</p>
        </div>
      ...
      >
        ...
        <div className="w-full md:w-1/2 flex flex-col border-b md:border-b-0 md:border-r border-border p-1 relative">
          <textarea 
            ...
            placeholder={mode === 'encode' ? "Enter text to encode to Base32..." : "Paste Base32 here..."}
            spellCheck="false"
          />
          {input && (
            <button 
        ...
        <div className="w-full md:w-1/2 flex flex-col p-1 relative bg-black/5 dark:bg-black/20">
          <textarea 
            ...
            readOnly
            placeholder={error ? "" : (mode === 'encode' ? "Base32 output will appear here..." : "Decoded text will appear here...")}
            spellCheck="false"
