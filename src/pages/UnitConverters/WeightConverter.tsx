...
const weightUnits = [
  { id: 'kg', name: 'Kilogram (kg)', toGram: 1000 },
  { id: 'g', name: 'Gram (g)', toGram: 1 },
  { id: 'mg', name: 'Milligram (mg)', toGram: 0.001 },
  { id: 'lb', name: 'Pound (lb)', toGram: 453.59237 },
  { id: 'oz', name: 'Ounce (oz)', toGram: 28.34952 },
];
...
export default function WeightConverter() {
  ...
  const calculate = (value: string, from: string, to: string, isForward: boolean) => {
    ...
    const fromFactor = weightUnits.find(u => u.id === from)?.toGram || 1;
    const toFactor = weightUnits.find(u => u.id === to)?.toGram || 1;
  ...
  return (
    <div className="relative flex flex-col items-center w-full min-h-full py-12 md:py-20 z-10">
      ...
      <div className="w-full max-w-2xl text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-bold text-text mb-4 tracking-tight">เครื่องมือแปลงน้ำหนัก</h1>
        <p className="text-muted font-medium text-lg">แปลงหน่วยน้ำหนักระหว่าง กิโลกรัม, ปอนด์, ออนซ์ และอื่นๆ ได้อย่างสะดวกง่ายดาย</p>
      </div>
